...
const commentUrl = "http://localhost:8081/v1/blog"; // get blog
...
const Comment = ({
  ...
  const handleSubmitReply = (reply) => {
    ...
    // Add the new comment to the commentData array (assuming this is how you're storing comments)
    // commentData.replies.unshift(newReply);
    console.log(commentData[id].replies.unshift(newReply));
...
const CommentSection = ({
  ...
  const fetchComents = async () => {
    const res = await axios.get(`${commentUrl}/3`);
    console.log(res.data);
    // const data = res.data;
    // console.log(data.object.comments);
    setCommentData(res.data.object.comments);
  };

  useEffect(() => {
    fetchComents();
    // const data = blog1.objectList[0].comments;
  }, []);
  ...
  useEffect(() => {
    ...
    let tot = commentData?.length;
    let extra = 0;

    for (let i = 0; i < tot; i++) {
      extra += commentData[i].replies.length;
    }

    setTotalComments(tot + extra);
  }, [commentData]);
